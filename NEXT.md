...
- [ ] **breaking 검토** — 26.05는 systemd initrd default → 부팅 경로 변화. release notes 확인 후 디바이스별 부팅 검증
- [ ] **순서 (리스크 차등)** — laptop/thinkpad(재설치 쉬움) → nuc(home server) → **oracle 마지막**(봇 런타임 서비스-크리티컬, 다른 데서 검증 끝낸 뒤)
