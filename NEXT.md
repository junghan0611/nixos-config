...
운영 baseline은 [AGENTS.md](AGENTS.md). 후속 작업 / 미완 검증은 여기에. 닫힌 항목은 [CHANGELOG.md](CHANGELOG.md)로 흘려보낸다 — 최근 스냅샷 `v2026.9.2-emacs.1`.
...
- [ ] **① `.deleted.*` 아카이브 이동 (6봇 합 32청크)** — dreaming B 와 **완전히 같은 패턴**(파일을 backups 로 옮기고 증분 색인, 가역). 위 1위 사례가 근거다. **효과 대비 위험 최저 — GLG 승인 대기.**
- [ ] **② 지배 세션 처리** — gpt `999d7150`(335/525) · glg 851청크 세션. 아카이브가 아니라 **살아있는 세션**이라 compact/reset 이 필요하고, ORACLE.md 의 `/model` 롤 사례와 같은 계열로 **라이브 대화를 굴릴 수 있다. GLG 판단 사안.**
