...
- Gateway: =https://oracle.tailb0e905.ts.net= (Tailscale Serve, tailnet 전용)
...
** Control UI 접근 — Tailscale Serve (로컬 머신에서)

tailnet 에 든 기기(thinkpad·안드로이드 앱)면 그냥 연다:

#+begin_src bash
# 브라우저: https://oracle.tailb0e905.ts.net/
#+end_src

*** ⚠️ 옛 SSH 터널은 이제 403 이다 (2026-09-08)

#+begin_src bash
# 아래는 더 이상 동작하지 않는다 — 기록용
ssh -N -L 18789:127.0.0.1:18789 <user>@<oracle-vm-ip>
#+end_src

터널과 호스트 루프백 트래픽은 도커 NAT 를 거쳐 컨테이너에 =172.19.0.1= 로 도착하는데,
그 IP 가 =gateway.trustedProxies= 항목이라 forwarded 클라이언트 헤더 없는 요청이
=403 proxy_attribution_required= 로 거부된다. =/32= 로 좁혀도 tailscale serve 와
같은 IP 로 뭉개져 분리되지 않는다. 전문은 =docs/openclaw-gotchas.md= 첫 항목.
