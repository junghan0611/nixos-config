...
#+begin_src bash
# 새 에이전트 추가 (채널 바인딩 포함)
docker compose run --rm openclaw-cli agents add work-agent \
  --bind mattermost \
...
| 채널 | 에이전트 | 용도 |
|------|----------|------|
| telegram | (기본) | 개인 어시스턴트 |
| mattermost | work-agent | 업무/자동화 허브 |
...
CLI 컨테이너와 Gateway 컨테이너가 별개여서 발생하는 일시적 현상.
Gateway 컨테이너가 =Up= 상태이면 무시 가능.
