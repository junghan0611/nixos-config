...
| ACP backend | `acpx` |
| 기본 fallback | `github-copilot/claude-sonnet-4.6` |
...
|---------|------|------------|------|-----------|
| main | github-copilot/claude-sonnet-4.6 | @junghan_openclaw_bot | GitHub Copilot (fallback) / ACPX Claude Opus | workspace (기본) |
...
| bbot | github-copilot/claude-sonnet-4.6 | @glg_b_bot | GitHub Copilot (fallback) / ACPX Claude Opus | workspace-bbot |

### 현재 운용 메모

- 기본 fallback은 Sonnet 4.6으로 유지해 비용/계정 리스크를 낮춤
- main, bbot은 필요한 대화방에서만 **ACPX + Claude Opus 4.6**으로 바인딩
- glg는 계속 Copilot Sonnet 4.6 유지
- GlueClaw plugin/마운트는 제거됨
- 비밀이 아닌 구조 변화는 `docker-compose.yml`과 운영 문서에 반영
