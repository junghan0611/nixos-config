...
| 채널 | Telegram (default + glg + gpt + gemini + mini + bbot), Discord, Mattermost |
| Claude direct 경로 | GlueClaw (`claude` CLI OAuth Max) |
| 세션 격리 | `per-account-channel-peer` (사용자별 독립 세션) |
| 세션 통신 | `sessions.visibility: all` |
...
|---------|------|------------|------|-----------|
| main | glueclaw/glueclaw-sonnet | @junghan_openclaw_bot | Claude CLI OAuth(Max) | workspace (기본) |
| glg | github-copilot/claude-sonnet-4.6 | @glg_junghanacs_bot | GitHub Copilot | workspace-glg |
| gpt | openai-codex/gpt-5.4 | @glg_gpt_bot | OpenAI OAuth | workspace-gpt |
| gemini | github-copilot/gemini-3.1-pro-preview | @glg_gemini_bot | GitHub Copilot | workspace-gemini |
| mini | github-copilot/gpt-5-mini | @glg_mini_bot | GitHub Copilot | workspace-mini |
| bbot | glueclaw/glueclaw-opus | @glg_b_bot | Claude CLI OAuth(Max) | workspace-bbot |

### 현재 운용 메모

- 기본봇(main)은 **GlueClaw Sonnet 4.6 direct runtime**으로 응답
- B봇(bbot)은 **GlueClaw Opus 4.6 direct runtime**으로 응답
- 예전 ACPX thread binding은 제거했고, stale ACP session state는 아카이브함
- GlueClaw plugin SSOT는 `~/repos/gh/glueclaw` 포크이며 compose에서 `/glueclaw`로 마운트
- 비밀이 아닌 구조 변화는 `docker-compose.yml`, `config/scripts/start-openclaw-gateway.sh`, `openclaw.json.reference`에 반영
