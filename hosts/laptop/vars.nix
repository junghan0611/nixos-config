{
  hostname = "nt930sbe-laptop-nixos";
  username = "junghan";
  email = "junghanacs@gmail.com";
  # Git/GitHub commit attribution — junghan0611 account, separate from the
  # real mail above. The noreply email maps commits to junghan0611 on GitHub
  # without exposing the real mail (which stays for email.nix/msmtp).
  gitName = "junghan0611";
  gitEmail = "31724164+junghan0611@users.noreply.github.com";
  sshKey = "ssh-rsa  rsa-key-20220215";
  locale = "en_US.UTF-8";
  timezone = "Asia/Seoul";
}
