...
- **API 아닌 Pro 쿼터**: `google-gemini-cli` OAuth = 구독 라우트(`usage: Pro 100% left · Flash 100% left`). 종량 `GEMINI_API_KEY`(google provider)와 분리. 회피는 **canonical `auth.order.google = ["google-gemini-cli:gtgkjh@gmail.com"]` 핀** — anthropic→claude-cli, openai→OAuth와 동일한 order 메커니즘. gemini 챗봇이 OAuth만 타는 것은 라이브 검증됨(`provider=google-gemini-cli`). `GEMINI_API_KEY`는 이미지(나노바나나)용으로 env 유지하되, **`auth.order.google`가 이미지 경로에 영향 주는지는 미재검증**(별도 generate_image 툴 경로라 무관할 가능성 큼 — soak 시 확인).
