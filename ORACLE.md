...
| glg (가족) | `openai/gpt-5.4` | `workspace-glg/` | partial | — | `@glg_junghanacs_bot`. Codex OAuth. 2026-06-10 5.5→5.4 강등(딱 할일만 — cross-DM 사건 후 판단 과잉 억제, 비용 1/2). active-memory 제외(응답성 우선) |
...
이미지 생성: `openai/gpt-image-2` via Codex OAuth (default since 2026-04-25). Google Imagen은 agent-directed 호출 시 사용 가능 (`GEMINI_API_KEY`로 banana/`gemini-3-flash-preview-image`). gemini 챗봇은 `auth.order.google` 핀으로 **OAuth(Pro 쿼터)만 탄다(라이브 검증)** — API키 안 씀. `GEMINI_API_KEY`는 이미지용으로 env 유지하되, `auth.order.google`의 이미지 경로 영향은 **미재검증**([NEXT.md §1](NEXT.md)).
