...
| **bbot** | `anthropic/claude-opus-4-8` | `workspace-bbot/` | off | ✓ | `@glg_b_bot`. claude-cli runtime native. 2026-06-13 fable-5 시도→**환원**(Fable 5가 구독/CLI에서 서빙 실패, auto-fallback이 deepseek를 잡아 정체성 훼손 → opus-4-8 환원). **fable 살아나면 GLG가 세션 `/model`로 수동 전환**(다른 모델 불필요). canonical `anthropic/claude-opus-4-8` + `agentRuntime.id=claude-cli` |
