...
| **bbot** | `anthropic/claude-opus-4-8` | `workspace-bbot/` | off | — | `@glg_b_bot`. claude-cli runtime native. 2026-06-13 active-memory 제외(recall 훅 mini lane이 매 direct 메시지마다 23~35s 2차 턴·절반 timeout으로 본 턴과 겹침→응답성 우선 제거, glg와 동일 처방). 2026-06-13 fable-5 시도→**환원**(Fable 5가 구독/CLI에서 서빙 실패, auto-fallback이 deepseek를 잡아 정체성 훼손 → opus-4-8 환원). **fable 살아나면 GLG가 세션 `/model`로 수동 전환**(다른 모델 불필요). canonical `anthropic/claude-opus-4-8` + `agentRuntime.id=claude-cli` |
...
운영 config:
- `agents: ["main", "gpt"]` — 2개 활성. glg 2026-06-09 제외(recall 훅이 가족 응답을 16~35s 지연). **bbot 2026-06-13 제외**(같은 mini-lane 증상: 매 direct 메시지마다 gpt-5.4-mini 2차 턴이 23~35s·절반 timeout으로 본 턴과 겹치고 `incomplete turn` 에러 노출 → 응답성 우선). mini/gemini 제외
